...
def load_data(from_file: str = 'csv', num_partitions: int = 1) -> dict:
    ...
    if filetype == 'csv':
        assert os.path.exists(
            from_file), f"File {from_file} does not exist! Ensure you are giving the relative path to the file. HINT: Your current directory is {os.curdir}"
        ...
        # For spacial concerns, limit amount of data we use
        pdf = pdf[:MAX_DATA_SIZE]
        ...
        for i, df in enumerate(list_of_partitions):
            ...
            dict_repr = {str(key).zfill(3): value for key,
                         value in dict_repr.items()}
            list_of_partitions[i] = dict_repr
